  ...
  socket.on('join_room', async (roomId: string) => {
    ...
    if (room.players.includes(socket.id)) {
      ...
      socket.emit('game_state', {
        ...
        scores: room.scores,
        currentPokemon:
          room.activePlayerId === socket.id ? room.currentPokemon : null, // Only send Pokemon to the active player
    ...
    console.log(
      `Player ${socket.id} joined room ${roomId}. Players: ${room.players.join(
        ', '
      )}`
    )
    ...
    if (room) {
      ...
      if (playerIndex !== -1) {
        ...
        console.log(
          `Player ${
            socket.id
          } left room ${roomId}. Remaining players: ${room.players.join(', ')}`
        )
  ...
  socket.on('submit_guess', async ({ roomId, guess }) => {
    ...
    if (isCorrect) {
      ...
      console.log(
        `Player ${
          socket.id
        } guessed correctly. Guessed players: ${room.guessedPlayers.join(', ')}`
      )
      ...
      if (room.guessedPlayers.length === room.players.length) {
        ...
        console.log(
          `All players have guessed. New active player: ${room.activePlayerId}`
        )
        ...
        while (
          room.guessedPlayers.includes(room.players[nextIndex]) &&
      ...
      // Player made a wrong guess, pass the turn to the next player and get a new Pokemon
      const currentIndex = room.players.indexOf(room.activePlayerId)
      const nextIndex = (currentIndex + 1) % room.players.length
      room.activePlayerId = room.players[nextIndex]

      // Get a new Pokemon for the next player
      const newPokemon = await getPokemon()
      room.currentPokemon = newPokemon

      console.log(
        `Player ${socket.id} guessed incorrectly. New active player: ${room.activePlayerId}`
      )

      // Notify the player who guessed incorrectly
      ...
      // Send the new Pokemon only to the new active player
      io.to(room.activePlayerId).emit('game_state', {
        players: room.players,
        scores: room.scores,
        currentPokemon: newPokemon,
      ...
      // Send a game_state update to the player who just guessed incorrectly
      io.to(socket.id).emit('game_state', {
        players: room.players,
        scores: room.scores,
        currentPokemon: null,
      ...
      if (playerIndex !== -1) {
        ...
        if (room.players.length === 0) {
          ...
          console.log(
            `Player ${
              socket.id
            } disconnected from room ${roomId}. Remaining players: ${room.players.join(
              ', '
            )}`
          )
          io.to(roomId).emit('player_left', {
